...
door.position.y = door.geometry.parameters.height / 2 - 0.05
house.add(door)
...
const tick = () => {
  ...
  const ghost1y = Math.abs(Math.sin(elapsedTime * 3))

  ghost1.position.set(ghost1x, ghost1y, ghost1z)

  const ghost2angle = -elapsedTime * 0.32
  const ghost2x = Math.cos(ghost2angle) * 8
  const ghost2z = Math.sin(ghost2angle) * 7
  const ghost2y = Math.abs(
    Math.sin(elapsedTime * 3) + Math.sin(elapsedTime * 2.5)
  )

  ghost2.position.set(ghost2x, ghost2y, ghost2z)

  const ghost3angle = -elapsedTime * 0.18
  const ghost3x = Math.cos(ghost3angle) * (7 + Math.sin(elapsedTime * 0.32))
  const ghost3z = Math.sin(ghost3angle) * (7 + Math.sin(elapsedTime * 0.5))
  const ghost3y = Math.abs(
    Math.sin(elapsedTime * 4) + Math.sin(elapsedTime * 2.5)
  )

  ghost3.position.set(ghost3x, ghost3y, ghost3z)
